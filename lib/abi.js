...
Ptr.prototype.vcall = function(method, args) {
    ...
    var f;
    // {{{
    // YOUR CODE HERE (put the function implementation into f)
    // }}}
    args.unshift(ptr.add(ventry.delta, ventry.type));
...
Ptr.prototype.cast = function(ty_to) {
    ...
    for (var i = 0; i < parents.length; i++) {
        var r = ptr.cast(parents[i]).cast(ty_to);
        if (r) return r;
    }
    return false;
}
