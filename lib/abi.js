...
Ptr.prototype.setMember = function(member, x) {
    ...
    {{{
        var off = Meta.getDataOffset(ptr.type, member);
        ptr.write(off, x);
    }}}
}
...
Ptr.prototype.vcall = function(method, args) {
    ...
    // and the appropriately adjusted pointer in shifted_ptr)
        var off = Meta.getVirtualOffset(ptr.type, method);
        var vtable = ptr.deref(0);
        var ventry = vtable[off];
        f = ventry.impl;
        shifted_ptr = Ptr.Ptr(ptr.array, ptr.offset + ventry.delta, ventry.type);
    }}}
...
Ptr.prototype.call = function(method, args) {
    ...
    {{{
    // ANSWER HERE (place the function in the variable f)
        f = Meta.getGlobal(ptr.type, method)
    }}}
    args.unshift(ptr);
    return f.apply(undefined, args);
}
